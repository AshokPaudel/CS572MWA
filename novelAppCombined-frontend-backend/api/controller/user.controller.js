...
const _validatePassword = function (foundUser,req,res,response){
    if(!foundUser){
        ...
        .then((isValidPwd)=>_handlePwdValidResult(isValidPwd,res,response))
        .catch((err)=>_handlePwdValidError(err,res,response))
        .finally(()=>sendResponse(res,response));
    }
}
const _handlePwdValidResult = function(isValidPwd,res,response){
    if(isValidPwd){
        console.log("Password validation Succeed ",isValidPwd);
